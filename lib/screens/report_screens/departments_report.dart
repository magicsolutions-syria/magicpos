...
class DepartmentReport extends ReportsScreen {
  static const String route = "/department-report-screen";

  const DepartmentReport({super.key});
  @override
  ReportsScreenState report() {
    return _DepartmentReportState();
  }
}

class _DepartmentReportState extends ReportsScreenState {
  @override
  bool button1() {
    return false;
  }
  ...
  Future<void> getNum() async {
    data = await db
        .readData("SELECT * FROM reports WHERE report_name = 'departments'");
    xRepNum = data[0]["X"];
    x2RepNum = data[0]["X2"];
    zRepNum = data[0]["Z"];
    z2RepNum = data[0]["Z2"];
...
super.getData();
    await getNum();
    data =
        await db.readData("SELECT * FROM 'departments' ORDER BY section_name");
  }
  ...
  Future<void> zero1() async {
    await db.changeData(
        "UPDATE 'departments' SET sold_quantity_one = 0,total_sells_price_one = 0.0");
  }

  @override
  Future<void> zero2() async {
    await db.changeData(
        "UPDATE 'departments' SET sold_quantity_two = 0 ,total_sells_price_two = 0.0 ");
  }
  ...
  Future<void> x2RepoNum() async {
    x2RepNum++;
    await db.changeData(
        "UPDATE 'reports' SET X2 = $x2RepNum WHERE report_name = 'departments'");
  }
  ...
  Future<void> xRepoNum() async {
    xRepNum++;
    await db.changeData(
        "UPDATE 'reports' SET X = $xRepNum WHERE report_name = 'departments'");
  }
  ...
  Future<void> z2RepoNum() async {
    z2RepNum++;
    await db.changeData(
        "UPDATE 'reports' SET Z2 = $z2RepNum WHERE report_name = 'departments'");
  }
  ...
  Future<void> zRepoNum() async {
    zRepNum++;
    await db.changeData(
        "UPDATE 'reports' SET Z = $zRepNum WHERE report_name = 'departments'");
  }
  ...
  List<Pair<String, Pair<int,int>>> getColumns() {
    ...
    Pair<String,Pair<int,int>> p4 = Pair("Print_Name_department", Pair(22,2));
    ...
    Pair<String,Pair<int,int>> p13 = Pair("section_name", Pair(17,0));
    Pair<String,Pair<int,int>> p14 = Pair(" Departments Report", Pair(0,0));
