// Copyright 2024 The saeed And hatem. All rights reserved.
// Use of this source code is governed by a ASH-style license that can be
// found in the 3mk file.
...
typedef SearchCallback<T> = int? Function(
    List<EditedDropdownMenuEntry<T>> entries, String query);
...
class EditedDropDownMenu<T> extends StatefulWidget {
  ...
  final List<EditedDropdownMenuEntry<T>> dropdownMenuEntries;
  ...
  @override
  State<EditedDropDownMenu<T>> createState() => _EditedDropDownMenuState<T>();
}

class _EditedDropDownMenuState<T> extends State<EditedDropDownMenu<T>> {
  final GlobalKey _anchorKey = GlobalKey();
  final GlobalKey _leadingKey = GlobalKey();
  late List<GlobalKey> buttonItemKeys;
  final MenuController _controller = MenuController();
  late bool _enableFilter;
  late List<EditedDropdownMenuEntry<T>> filteredEntries;
  ...
  void initState() {
    ...
    _menuHasEnabledItem = filteredEntries
        .any((EditedDropdownMenuEntry<T> entry) => entry.enabled);

    final int index = filteredEntries.indexWhere(
        (EditedDropdownMenuEntry<T> entry) =>
  ...
  @override
  void didUpdateWidget(EditedDropDownMenu<T> oldWidget) {
  ...
  List<EditedDropdownMenuEntry<T>> filter(
      List<EditedDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String filterText = textEditingController.text.toLowerCase();
    return entries
        .where((EditedDropdownMenuEntry<T> entry) =>
            entry.label.toLowerCase().contains(filterText))
        .toList();
  }

  int? search(List<EditedDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }
    final int index = entries.indexWhere((EditedDropdownMenuEntry<T> entry) =>
        entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<EditedDropdownMenuEntry<T>> filteredEntries,
      TextDirection textDirection,
      {int? focusedIndex, bool enableScrollToHighlight = true}) {
    final List<Widget> result = <Widget>[];
    for (int i = 0; i < filteredEntries.length; i++) {
      final EditedDropdownMenuEntry<T> entry = filteredEntries[i];
  ...
  Widget build(BuildContext context) {
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder:
        ...
        final Widget textField = KeyboardListener(
            focusNode: _focus,
            child: TextField(
                key: _anchorKey,
                ...
                onEditingComplete: () {
                  if (currentHighlight != null) {
                    final EditedDropdownMenuEntry<T> entry =
